...
Why would hyperbolic embeddings fare worse the further we get from the training labels? In an attempt to explore this, browsed through several concrete predictions. Figure \ref{fig:eucgoodzsl} gives an example of a zero-shot class on which Deep-Glove does well and Deep-HSEM poorly, namely 'ski jumping'. The closest class in the training data is probably the word 'ski' which gives several images similar to those shown here in that they include wintery scenes and people with skis. The contextual similariy of the Euclidean embedding has 'ski' close to many related terms and thus the model can extrapolate appropriately, though as a consequence some non-sensical predictions are made e.g. 'ski-lodge'. The reason why our hyperbolic model fails on such examples is likely due to the rigidity of the WordNet hierarchy, in which 'ski jumping' is under a super-category of 'sports', while the most relevant training label 'ski' is under 




The opposite case is displayed in Figure \ref{fig:poincgoodzsl}.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{eucGoodzsl}
...
Finally, the overall accuracy of all the models is very low which portrays the difficulty of the task as a whole.
