...
The aim of training an ANN is based on learnng model weights that minimize some appropriate loss function, such as the cross-entropy loss above. Most supervised deep learning models, including the basic feedforward-network described above, are nowadays trained using the back-propagation algorithm \cite{Linnainmaa1976} \cite{Rumelhart1985}. The main idea of this algorithm is to use the chain rule to decompose the gradient of a loss function so that it can be efficiently passed back through the network. More formally, if $\nabla_{\theta}L(\theta)$ denotes the gradient of the loss with respect to all model parameters, then applying the chain rule we get:

\begin{align} \label{BP_eq}
  \nabla_{\theta}L(\theta) = \left(\frac{\partial \mathbf{f}}{\partial \mathbf{\theta}}\right)^{\top}
\end{align}
