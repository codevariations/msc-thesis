...
\caption{Small excerpt of Robust Zero-Shot Classification. In this task the models are rewarded for predicting the correct label or one of the correct higher-level categories of the image. Performance is measured with inverse distance-weighted top-$k$ accuracies; predictions closer to the original label are credited more. The hierarchical 2011 20k datasets were derived from Experiment 1 by adding four levels of superordinate labels to each original label from the WordNet hierarchy.}
\label{tbl:smallrob}
\end{table}
...
\begin{figure}[!t]
  \centering
	...
	\caption{An example of an image and model predictions on the \textit{robuse zero-shot classification} task which measures how well models are able to predict semantic hierarchy of visual objects. This handpicked example illustrates the robustness of the Deep-HSEM model.}
	\label{fig:smallx}
\end{figure}
