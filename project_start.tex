...
Overall, the findings here corroborate our hypothesis that the hyperbolic embedding models are able to exploit the relationships between visual and semantic hierarchies, which enables them to make more robust higher-level predictions. Figure \ref{fig:robres} displays a few example images and predictons that illustrate this. Take as an example the first image of a 'puppy': eventhough Deep-HSEM is unable predict this exact label, it succesfully classifies the image as a dog and canine. Deep-Glove on the other makes very granular predictions. It's prediction 'springer spaniel' does appear correct

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{robresx}
  \caption{Four example test images from the robust zero-shot classification task. The first three illustrate succesfull high-level predictions by Deep-HSEM. Deep-Glove makes classifications at a very granular level and also makes more completely incorrect predictions. The fourth image is an example of a failure case for both models.}
  \label{fig:robres}
\end{figure}
