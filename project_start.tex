...
\begin{table}[]
  ...
  \multicolumn{1}{l}{\textbf{Classificaton}} & & \multicolumn{5}{l}{\textbf{Accuracies(\%)}}  \\
  \midrule
...
Another common issue encountered in zero-shot learning is that of the domain projection shift \cite{Fu2015}; the training and test classes in zero-shot learning are fully disjoint and thus the learnt mapping may not transfer well. We suspect that this issue might be relevant for hyperbolic embeddings. In particular, the hierarchical organization means that the embedding space is not populated evenly, and therfore the model learns a mapping to a more constrained area. This could explain why the hyperbolic models had lower accuracy on the standard zero-shot classification experiment than the Euclidean baselines, especially on classes that were far away from the training labels. Future research should investigate how this could be alleviated.
