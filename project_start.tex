...
\underlin{Learning and Using Taxonomies For Fast Visual Categorisation - Griffin and Perona 2008}\\
\textit{Overview} \\
...
\newpage
\textbf{ZERO-SHOT LEARNING AND RELATED PAPERS} \\
...
\underline{Large Scale Image Annotation: Learning to Rank with Joint Word-image Embeddings - Weston et al. 2010} \\
\textit{Overview}\\
...
\underline{Zero-shot Learning Through Cross-modal Transfer - Socher et al. 2013} \\
\textit{Overview} \\
...
DeViSE extends work of Weston et al. (above) as it allows non-linearities and also capture semantics from text that's not contained in the image lables, hence allowing for zero-shot generalization. It also improves on the Socher et al. paper by using a deep model and avoid the trade-off which that paper has in predicting seen and unseen classes. That model also had several different models whilst this one uses a unified model that only uses embeddings.
