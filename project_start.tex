...
\end{align*}
Here $\mathbf{v}_t$ are the embedding vectors expressed in Klein coordinates. Since we used the Poincare model to construct our embeddings, we had to translate them into Klein coordinates,$\mathbf{f}_t \rightarrow \mathbf{v}_t$, using the Equation \ref{eq:p2k} introduced in the previous section. $\gamma(\mathbf{v}_t)$ are known as Lorentz factors: $\gamma(\mathbf{v}_t)=\frac{1}{\sqrt{1 - \lVert\mathbf{v_t}\rVert^2}}$. Above expositon followed \cite{Gulcehre2018}, who, in turn, based it on Ungar \cite{Ungar2005} where the full derivation can be found. 

In summary the Convex-HSEM consists two step. First, the pre-trained VGG-16 model is used to produce predicted class probabilities, out of which $T$ most likely ones are used to calculated a convex combination of 
