...
\section{Ideas for Further Research}



Main contribution of this work is therefore.

Two possible issues. First is are possible problem of mapping from image space into the hyperbolic space, and the second one is that the eventhough semantic space may not have visual smoothness.

\subsubsection{issues of mapping into embedding space}


On the otherhand, our ConSE model failed but this may be because Einstein midpoint is not great..

presenting the entire network in hyperbolic space since CNN learn hierarchical representations in layers, it might be better to have everything in hyperbolic space and then train the entire model on Riemannian optimization.
...
final layer of CNN doesnt have necessarily enough detailed information contained. Perhaps need to make links to higher layers \cite{Ba2015}
...
problem with riemannian and Euclidean gradiens. and how riemannian graients are calculated.
Numerical issues, attempted to use double-precision (64 bit floating points.

\subsubsection{Issues of visual smoothness}

GCN showed however accounting excplicit semantic hierarchy is important. Nevertheless, at the same time their model also used similarity emebedding. Likely both are important consider bethos example vs. words that dont have have visual relevance, and also biological taxonomy. perhaps use both models.
...
maybe one contribution is that visual similarity is not entire from taxonomy e.g. zebra maybe difficult to guess. Our benefit is really probabily to predict basic level classes. 

Essentially our model proposes that a hyperbolic embedding is better repsentation of visual similarity than Euclidean similarity based embeddings buil on large text corpus, such at the Wikipedia data set. Our experiments suggest that both have advantages. A relatd issue is then the visual smoothness of poincare embedding space. Benthos example.
Nickel showed that the Poincare embedding much better represent relationships of explicit graph.
...
problem is that we are training only transformation to the leafs of the nodes. I wonder if we could includ basic level labels since those are the ones humans learn first and also some suggest on average capture most distinction (see the two papers)
we probably need to consider domain adaptation in more detail e.g. some literature \cite{Patel2015}, \cite{Wang2018}
