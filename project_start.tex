...
\section{Qualitative Overview:} We propose two model architectures Deep Hyperbolic Semantic Embedding Model (Deep-HSEM) and Convex Hyperbolic Semantic Embedding Model (Convex-HSEM). The general intuition is to replace the standard one-hot-encoded targets used in deep learning with poincare embedding vectors that should better reflect the location of each label in a hierarchical semantic taxonomy; we use the lexical database WordNet as the hierarchy \cite{Miller1995}. The motivation for this approach is based on our hypothesis that semantic hierarchy corresponds to hierarchy of visual features: for instance, wolves and dogs share many visual features as both fall under a common hypernym (super-category) Canines. Implicit of this hypothesis is that the semantic hierarchy reflects the different categorisations that exist in nature such as that of biological taxonomy which reflects the principle of common descent and thus shared visual attributes. Similarly, for human-made objects, visual similarity tends to increase as two objects become more similar in what they are intended to be used for. Crucially, we hypothesize that a hyperbolic embedding space allows us to represent these hierarchies better than pure similarity based word embeddings which just captures the similarity of the words' contexts. As an example, the words 'car' and 'road' may occur relatively close to each other in an embedding that is built on a large text corpuses, such as Wikipedia, but the two in isolation are not visually alike, which can make it harder to learn the mapping from visual features into the semantic space. In fact, 
