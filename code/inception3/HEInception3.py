...
__all__ = ['HEInception3', 'he_inception_v3']
...
#select only those weights which we in the new model

def he_inception_v3(pretrained=False, **kwargs):
    #Inception v3 for projecting into hierarchical poincare embedding 
    #Inception v3 model architecture from <http://arxiv.org/abs/1512.00567>
    #Args: pretrained (bool): If True, returns a model pre-trained on ImageNet
    if pretrained:
        if 'transform_input' not in kwargs:
            kwargs['transform_input'] = True
        model = HEInception3(**kwargs)
        #since we changed some layers of the original model
        #we need to ensure only load corresonding layers
        model_dict = model.state_dict()
        pretrained_dict = model_zoo.load_url(model_urls['inception_v3_google'])
        filtered_dict = {k: v for k, v in pretrained_dict.items() if k in
                         model_dict.keys()}
        model_dict.update(filtered_dict)
        model.load_state_dict(model_dict)
        return model
    return HEInception3(**kwargs)


class HEInception3(nn.Module):

    def __init__(self, num_classes=1000, num_emb_dims=5, aux_logits=True,
        ...
        self.dense = nn.Linear(2048, num_emb_dims)
...
class InceptionD(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.branch7x7x3_3 = BasicConv2d(192, 192,
                kernel_size=(7, 1), padding=(3, 0))
        self.branch7x7x3_4 = BasicConv2d(192, 192,
...
class InceptionAux(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, num_emb_dims):
        ...
        self.dense = nn.Linear(768, num_emb_dims)
