...
def main():
    global args, best_prec1, w2v_emb, imgnet_glov_wgt, imgnet_glov_labels
    ...
    with open('w2v_emb.pkl', 'rb') as f:
        w2v_emb = pkl.load(f, encoding='latin1')
    ...
    #Change model to project into w2v space
    ...
    #create glove embedding that only contains imagenet synsets
    imgnet2glov_idx = [w2v_emb['objects'].index(i) for i in imgnet_classes]
    imgnet_glov_wgt = torch.tensor(w2v_emb['model'], dtype=torch.float)[[imgnet2glov_idx]]
    imgnet_glov_labels = [w2v_emb['objects'][i] for i in imgnet2glov_idx]
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        target_emb_idx = [imgnet_glov_labels.index(i) for i in target_ids]
        target_embs = imgnet_glov_wgt[[target_emb_idx]]
        target_embs = target_embs.cuda(args.gpu, non_blocking=True)

        # compute output
        output = model(input)
        loss = criterion(output, target, imgnet_glov_wgt)

        # measure accuracy and record loss
        prec1, prec5 = accuracy(output, imgnet_glov_wgt, target, topk=(1, 5))
...
def save_checkpoint(state, is_best, filename='checkpoint_w2v.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_w2v.pth.tar')
