...
from custom_loss_allemb import PoincareXEntropyLoss
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        matching_poinc_targets = torch.gather(torch.tensor(
                           imgnet2poinc_idx, dtype=torch.long).cuda(), -1, target)
        target_poinc_idx = matching_poinc_targets.long().cuda()

        # compute output
        output = model(input)
        loss = criterion(output, target_poinc_idx, imgnet2poinc_idx,
                         poinc_emb['model']['lt.weight'])
...
def save_checkpoint(state, is_best, filename='checkpoint_allemb.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_allemb.pth.tar')
